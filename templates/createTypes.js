...
function getObjectFields(modelObject, isInputType) {
	var attributes = modelObject._attributes;
	var queryName = helpers.getName(modelObject).queryName;
	...
	return function () {
		return Object.keys(attributes).reduce(function (fields, fieldName) {
			...
			var throughModel = temp.through;
			var fieldTypeName = void 0;
			var childModel = void 0;
            var fieldType;
            var fieldConnectionTypeName;
			...
			if (model || collection) {
				...
				fieldTypeName = isInputType ? helpers.getName(childModel).inputTypeName : helpers.getName(childModel).typeName;
			}
...
function createOutputType(model) {
	...
	var typeName = helpers.getName(model).typeName;
...
function createInputType(model) {
	// Define output type name
	var inputTypeName = helpers.getName(model).inputTypeName;
